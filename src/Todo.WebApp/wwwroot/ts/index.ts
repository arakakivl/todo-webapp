...
async function getUncompletedItems() {
    await getItems(mainApiUrl + "?state=uncompleted").then(x => putItemInItemsArea(x));
}

async function getCompletedItems() {
    await getItems(mainApiUrl + "?state=completed").then(x => putItemInItemsArea(x));
}
...
async function checkToDo(item : Item) {
    await fetch(mainApiUrl + item.id, {
        method: "PATCH",
